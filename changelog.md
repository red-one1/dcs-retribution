...
* **[Mission Generator]** Dynamically allocated TACAN channels no longer collide with map DME/VOR-DME beacons (only TACAN/VORTAC beacons were blacklisted; DME and VOR-DME share TACAN's channelization, so their channels were left available for reuse). (#36)
* **[Mission Generator]** Map beacons whose DCS data omits a TACAN channel (e.g. Syria's KALDE "KAD" VOR-DME) are no longer silently skipped when blacklisting occupied channels: the channel/band is now derived from the beacon's VHF frequency per the standard ICAO VOR/TACAN channelling plan, which also corrects the band (previously always assumed X) for beacons that did have a channel set. (#36)
* **[UI]** The "Assign TACAN" dialog now warns in real time ("Already in use!") when the selected channel/band is already used by a map beacon, another carrier/airfield, or a flight, instead of only flagging it after saving. (#36)
* **[UI]** Fixed the "Assign TACAN" warning not firing for a carrier/airfield's own already-conflicting channel: excluding "my own current assignment" from the in-use set was matching by value, which also hid a real beacon conflict whenever it happened to share that same channel number. (#36)
