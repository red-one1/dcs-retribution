...
splash_damage_options = {
    ...
    ---------------------------------------------------------------------- Retribution Additions ------------------------------------------------------------
    ["shipRadarDamageEnable"] = false, --Ship radars turn off when hit by HARM/anti-radiation missiles (Retribution)
    ["oca_aircraft_damage_boost"] = 3000, --apply extra wave-explosion damage to parked airplanes/helicopters (Retribution OCA/Aircraft missions)
...
-- Retribution: anti-radiation missiles that disable ship radar emissions (used by shipRadarDamageEnable)
...
-- Retribution: height above ground level for an object (used by oca_aircraft_damage_boost)
function getAGL(object)
...
explTable = {
	...
	["AGM_62_I"]  = 274, --1000lb warhead. AGM-62 Walleye I. F-4E
	["AGM_62_II"]  = 400, --2000lb warhead. AGM-62 Walleye II ER. F-4E
                                ...
                                world.searchObjects({Object.Category.UNIT, Object.Category.STATIC}, volS, function(obj)
                                    ...
                                    if dist > 1 then
                                            ...
                                            if damage_for_surface > splash_damage_options.cascade_damage_threshold then
                                                ...
                                                -- Retribution: extra damage to parked airplanes/helicopters to make OCA/Aircraft missions viable
                                                if isUnit and (obj:getDesc().category == Unit.Category.AIRPLANE or obj:getDesc().category == Unit.Category.HELICOPTER) and (obj:inAir() == false or getAGL(obj) < 50) then
                                                    explosion_size = intensity * splash_damage_options.oca_aircraft_damage_boost
                                                end
                                                -- Retribution: disable ship radar emissions when hit by anti-radiation missiles
                                                if splash_damage_options.shipRadarDamageEnable and isUnit and obj:getDesc().category == Unit.Category.SHIP and antiRadiationMissile[wpnData.name] ~= nil then
                                                    obj:enableEmission(false)
                                                    if splash_damage_options.debug then
                                                        debugMsg("BDA: " .. obj:getTypeName() .. " radar destroyed")
                                                    end
                                                end
